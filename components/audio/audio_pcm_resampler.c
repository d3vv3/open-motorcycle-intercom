...
static int32_t clamp_output_q16(int64_t value, int32_t positive_limit_ppm)
{
    ...
    const int64_t negative_limit =
        -(int64_t)AUDIO_PCM_RESAMPLER_MAX_CORRECTION_PPM * 65536;
...
static void update_controller(audio_pcm_resampler_t *resampler, size_t depth,
                              bool recovery_active)
{
    ...
    const int32_t positive_limit_ppm = recovery_active
                                           ? AUDIO_PCM_RESAMPLER_MAX_RECOVERY_PPM
                                           : AUDIO_PCM_RESAMPLER_MAX_CORRECTION_PPM;
    const int64_t positive_limit_q16 = (int64_t)positive_limit_ppm * 65536;
    const int64_t negative_limit_q16 =
        -(int64_t)AUDIO_PCM_RESAMPLER_MAX_CORRECTION_PPM * 65536;
    ...
    proportional_q16 =
        (int64_t)error_q8 * PROPORTIONAL_GAIN_PPM_PER_SAMPLE * 256;
    candidate_integrator = resampler->integrator_q16 +
                           (int64_t)error_q8 * INTEGRAL_GAIN_Q16_PER_ERROR_Q8;
    ...
    resampler->correction_ppm = clamp_output_q16(
                                        proportional_q16 + resampler->integrator_q16,
                                        positive_limit_ppm) /
                                    65536;
}
                                ...
                                int16_t output[AUDIO_PCM_RESAMPLER_BLOCK_SAMPLES])
{
    ...
    for (i = 0; i < AUDIO_PCM_RESAMPLER_BLOCK_SAMPLES; ++i) {
        const int64_t remaining =
            (int64_t)(AUDIO_PCM_RESAMPLER_BLOCK_SAMPLES - 1u - i);
    ...
    audio_pcm_resampler_t *resampler, const int16_t *samples, size_t sample_count, bool active)
{
    ...
    if (first_count != 0u) {
        memcpy(&resampler->ring[resampler->write_index], samples,
               first_count * sizeof(samples[0]));
    ...
    int16_t output[AUDIO_PCM_RESAMPLER_BLOCK_SAMPLES], size_t upstream_samples)
{
    ...
    update_controller(resampler, resampler->depth + upstream_samples,
                      result.recovery_active);
    result.correction_ppm = resampler->correction_ppm;
    step_adjustment_q32 = (int64_t)resampler->correction_ppm *
                          (int64_t)PHASE_ONE_Q32 / 1000000;
    step_q32 = (uint64_t)((int64_t)PHASE_ONE_Q32 + step_adjustment_q32);
    last_phase = resampler->phase_q32 +
                 step_q32 * (AUDIO_PCM_RESAMPLER_BLOCK_SAMPLES - 1u);
    ...
    if (required > resampler->depth) {
        ...
        resampler->filtered_depth_q8 =
            (int32_t)AUDIO_PCM_RESAMPLER_TARGET_SAMPLES * 256;
    ...
    for (i = 0; i < AUDIO_PCM_RESAMPLER_BLOCK_SAMPLES; ++i) {
        ...
        int16_t sample = interpolate(resampler->ring[ring_offset(resampler, offset)],
                                     resampler->ring[ring_offset(resampler, offset + 1u)],
                                     fraction);
    ...
    resampler->read_index =
        (resampler->read_index + consumed) % AUDIO_PCM_RESAMPLER_RING_SAMPLES;
