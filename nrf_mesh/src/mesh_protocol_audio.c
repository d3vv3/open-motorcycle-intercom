...
{
    uint8_t depth = s_tx_head >= s_tx_tail ? (uint8_t)(s_tx_head - s_tx_tail)
...
static void note_tx_starvation(void)
{
    if (mesh_tx_metrics_should_log_starvation(s_stat_tx_starvation)) {
        ...
        printk("[TXSTARVE] total=%u r=%u id=%u sl=%d q=%u mq=%u tts=%d\n", s_stat_tx_starvation,
               s_role, s_node_id, s_slot_index, s_tx_queue_depth_dbg,
...
static bool deferred_tail_has_proven_successor(void)
{
    ...
    if (tail->packet_type != MESH_PKT_AUDIO_V2 || successor->packet_type != MESH_PKT_AUDIO_V2 ||
        !audio_bundle_parse(tail->data, tail->len, &tail_bundle) ||
        !audio_bundle_parse(successor->data, successor->len, &successor_bundle)) {
        return false;
    }

    return mesh_core_successor_carries_prev1(s_local_deferred_seq, &tail_bundle, &successor_bundle);
}
                                   ...
                                   bool *prev2_forwarded)
{
    ...
    if (hdr->payload_len != (uint16_t)(*packet_len - sizeof(*hdr))) {
        s_stat_bundle_bad++;
        return -EINVAL;
    }
    ...
    *prev2_forwarded = bundle.previous2_len != 0u;
     ...
     * audio_bundle_strip_oldest() removes exactly the stripped frame's payload
     * bytes, so the lengths are known without mutating the packet. */
    ...
    int strips = mesh_core_fit_airtime(remaining_us, AUDIO_TX_MARGIN_US, ESB_NORMAL_RAMP_US,
                                       ESB_2MBPS_US_PER_BYTE, ESB_2MBPS_OVERHEAD_BYTES, candidates,
                                       candidate_count);
    /* A late drop (-1) still strips every predecessor first so the strip
     * counters describe what the relay attempted. */
    ...
    if (bundle_valid) {
        return esb_radio_send(packet, *packet_len);
    }
    return -EINVAL;
}
                                                  ...
                                                  uint8_t tx_flags, bool retain_prev1)
{
    ...
    if (entry->packet_type == MESH_PKT_AUDIO_V2) {
        ...
        if (!audio_bundle_parse(bundle_data, bundle_len, &bundle)) {
             ...
             * prev1 (see transmit_relay_audio_v2 for the length model). */
            uint32_t candidates[3];
            size_t candidate_count = 0;
            ...
            int strips = mesh_core_fit_airtime(remaining_us, AUDIO_TX_MARGIN_US, ESB_NORMAL_RAMP_US,
                                               ESB_2MBPS_US_PER_BYTE, ESB_2MBPS_OVERHEAD_BYTES,
            ...
            note_audio_activity(s_node_id, bundle.flags & AUDIO_BUNDLE_FLAG_CURRENT_ACTIVE);
            if (bundle_valid && strips < 0) {
                s_stat_bundle_late_drop++;
                ret = -ETIME;
                ...
                ret = mesh_protocol_tx_send_packet_ex(MESH_PKT_AUDIO_V2, bundle_data,
                                                      (uint16_t)bundle_len, MESH_AUDIO_TTL_DEFAULT,
...
static void slot_tx_handler(uint8_t slot_index, uint32_t frame_counter)
{
    ...
    if (local_pending && relay_pending && s_relay_contention_turn) {
        defer_local =
            mesh_core_defer_local_tail(local_pending, relay_pending, s_relay_contention_turn,
...
static void drain_audio_ingress(void)
{
    ...
    while (k_msgq_get(&s_audio_ingress_queue, &entry, K_NO_WAIT) == 0) {
        (void)process_audio_ingress(entry.data, entry.len, entry.audio_flags, entry.packet_type);
    }
}
